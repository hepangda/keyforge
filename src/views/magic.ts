...
export function renderMagicRequestPage(
  ...
  const body = `<div class="card-page"><a class="btn btn--ghost btn--sm btn--auto back-link" href="${escapeHtml(continuationHref("/login", returnTo, reauthenticating, hint))}">${escapeHtml(i18n.t("Back to sign-in methods"))}</a><main class="card">
  <div class="head">
    ${brandHeader()}
  ...
  <form method="post" action="/login/magic" autocomplete="on">
    ...
    <label class="field">
      ...
      <input class="input" type="email" name="email" required autocomplete="username"
        autocapitalize="none" spellcheck="false" autofocus placeholder="you@example.com" value="${escapeHtml(email)}">
    </label>
...
export function renderMagicSentPage(
  i18n: I18n,
  email: string,
  returnTo: string,
  reauthenticating: boolean,
  ...
  <p class="form-hint">${escapeHtml(i18n.t("Check your spam folder if the email doesn't arrive."))}</p>
  <p class="foot"><a class="link-quiet" href="${escapeHtml(continuationHref("/login", returnTo, reauthenticating, hint))}">${escapeHtml(i18n.t("Return to sign-in methods"))}</a></p>
</main>`
...
export function renderMagicConfirmation(
  ...
  <div class="callout">${escapeHtml(i18n.t("This link expires in 15 minutes. Sign in before it does."))}</div>
  ...
  <p class="foot"><a class="link-quiet" href="${escapeHtml(continuationHref("/login", returnTo, reauthenticating))}">${escapeHtml(i18n.t("Cancel"))}</a></p>
</main>`
