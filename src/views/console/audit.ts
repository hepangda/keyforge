...
function filterInput(label: string, name: string, value: string): string {
  return `<label class="field"><span class="field__label">${escapeHtml(label)}</span><input class="input" type="text" name="${name}" value="${escapeHtml(value)}"></label>`
}
...
export function renderAuditList(
  ...
  const content = `<section class="panel">
    <div class="panel__head"><div><h2 class="panel__title">Audit log</h2><p class="panel__desc">Every security-relevant event, newest first.</p></div></div>
    <div class="panel__body">
      <form method="get" action="/console/audit" class="audit-filters">
        ${filterInput("Event type", "event_type", filters.eventType)}
        ${filterInput("User ID", "user_id", filters.userId)}
        ${filterInput("Client ID", "client_id", filters.clientId)}
        ${filterInput("Actor user ID", "actor_user_id", filters.actorUserId)}
        ${filterInput("Actor client ID", "actor_client_id", filters.actorClientId)}
        <div class="audit-filters__actions">
          <button class="btn btn--primary btn--sm btn--auto" type="submit">Apply</button>
          <a class="btn btn--ghost btn--sm btn--auto" href="/console/audit">Clear</a>
        </div>
      </form>
    </div>
    ${dataTable(["Time", "Event", "Result", "Actor user", "Actor client", "Subject user", "Subject client", "Detail"], rows, "No matching events.")}
    <div class="panel__body">${pager(baseHref(filters), limit, offset, logs.length, hasNext)}</div>
  </section>`
  return consoleShell("Audit log — Admin console", chrome, content)
}
