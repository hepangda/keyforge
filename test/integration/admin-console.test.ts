...
describe("console access control", () => {
  ...
  it("forbids non-admin users with 403", async () => {
    const user = await createUser(env, { email: "nobody@pangda.app", userType: "external" })
  ...
  it("renders the console overview for an admin", async () => {
    const session = await loginAs("admin", "admin")
    const res = await SELF.fetch(`${ISSUER}/console`, {
      headers: { cookie: `__Host-keyforge_session=${session}` },
    })
    ...
    expect(page).toContain('<main class="shell-main">')
    expect(page).toContain('<h1 class="sr-only">Overview</h1>')
  })
...
describe("console users", () => {
  ...
  it("re-renders invalid user creation values but always clears the password", async () => {
    ...
    const page = await res.text()
    expect(page).toContain("Initial passwords must contain 12–128 characters.")
    ...
    expect(page).toContain('<option value="internal" selected>Internal</option>')
  ...
  it("disables a user through the edit form", async () => {
    const target = await createUser(env, { email: "target@pangda.app", userType: "external" })
    const session = await loginAs("admin", "admin")
    const csrf = await consoleCsrf(session)
  ...
  it("revokes a user's sessions", async () => {
    const target = await createUser(env, { email: "target2@pangda.app", userType: "external" })
...
describe("console clients", () => {
  ...
  it("re-renders every non-secret new-client field with a specific redirect error", async () => {
    ...
    expect(page).toContain('<option value="confidential" selected>')
    expect(page).toContain('<option value="application" selected>')
